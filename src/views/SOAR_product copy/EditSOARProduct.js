...
export default function EditSOARProduct({ toggleSidebar, id }) {
  ...
  const { SOAR_product, loading: SOAR_loading } = useSelector((state) => state.soarProducts) // Use the correct state name
  const { error, isUpdated, loading } = useSelector((state) => state.updateSOARProduct) // Ensure the state names match
  ...
  useEffect(() => {
    dispatch(getSOARProduct(id)) // Fetch product by ID
  ...
  }, [dispatch, isUpdated, error, navigate]) // Add `navigate` to dependencies

  useEffect(() => {
    if (SOAR_product) {
      setUser({
        product_name: SOAR_product.product_name,
        extra_data_field: SOAR_product.extra_data_field || [], // Ensure these are arrays
  ...
  const addRowSet = () => {
    setUser((prevUser) => ({
      ...prevUser,
      field_in_log: [...prevUser.field_in_log, ''],
      extra_data_field: [...prevUser.extra_data_field, ''],
    }))
  }
  ...
  const submit = () => {
    if (validate_required_keys(user, setValidationError)) {
      dispatch(updateSOARProduct(id, user))
    }
  }
            ...
            <CCol xs>
              <div className="update-SOAR-product" style={{ borderTop: '2px solid #303030' }}>
                <CForm className="p-3">
                    ...
                    >
                      ...
                      <CTableBody>
                        {user.field_in_log.map((_, index) => (
                          <CTableRow key={index}>
                            {' '}
                            {/* Use index for key since it's stable in this context */}
                  ...
                  <CButton
                    size="sm"
                    type="button" // Changed to "button" for better semantics
                    color="primary"
                    variant="outline"
                    style={{
                      fontSize: 13,
