...
export default function EditLoggerProduct({ toggleSidebar, id }) {
  const dispatch = useDispatch()
  const { logger_product, loading: logger_product_loading } = useSelector(
    (state) => state.logger_product,
  )
  const { isUpdated, loading, error } = useSelector((state) => state.update_logger_product)
  ...
  useEffect(() => {
    if (logger_product) {
      setProduct({
        ...product,
        product_name: logger_product.product_name,
        product_type: logger_product.product_type,
        log_type: logger_product.log_type,
        template_name: logger_product.template_name,
        status: logger_product.status,
        queue_size: logger_product.queue_size,
        max_disk_space: logger_product.max_disk_space,
        worker_threads: logger_product.worker_threads,
        filename: logger_product.filename,
        keywords: logger_product.keywords,
      })

      setNewKeywords(logger_product.keywords)
    }
  }, [logger_product])
  ...
  const submit = () => {
    if (validate_required_keys(product, setValidationError)) {
      ...
      dispatch(updateLoggerProduct(id, product))
    }
  }
